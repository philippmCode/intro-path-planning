...
class SamplerVisualizer:
    """
    Visualizes the sampling process standalone. 
    Plots the origin edge, newly sampled nodes, obstacles, and free segments.
    """
    ...
    def plot_sampling(self, current_nodes, current_segment=None, past_history=None, 
        ...
        # 1. Plot the obstacles (background layer 1)
        checker.drawObstacles(ax)

        # 2. Plot the verified free segments (yellow lines)
        if free_segments:
        ...
        # 3. Plot the historical edges and nodes (background layer 2)
        ...
        # 4. Plot the current origin edge (foreground layer)
        ...
        # 5. Plot the current sampled nodes
        if current_nodes:
            ...
            ax.scatter(x_coords, y_coords, color='blue', alpha=0.9, 
                       s=40, label='New Sampled Nodes', zorder=6)
        ...
        ax.legend(handles=handles, labels=labels, loc='upper right', bbox_to_anchor=(1.35, 1))
        ax.set_aspect('equal')
